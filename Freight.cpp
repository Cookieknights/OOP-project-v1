...
void Freight::decCapacity() {
    capacity--;
}

void Freight::showFreight() {
    //For debug
...
void Freight::deleteFreight(vector<Freight>& freightList, bool& newupdate) {
    string deletefline;
    int freightInd = 0;
    bool ffound = false;
    ...
    while (ffound == false && deletefline != "-1") {
        if (deletefline[0] != 'F') {
            ...
            for (int i = 0; i < freightList.size(); i++) {
                if (freightList[i].getID() == deletefline) {
                    freightInd = i;
                    ffound = true;
...
void Freight::editFreight(vector<Freight>& freightList, bool& newupdate) {
        ...
        case 1:
            ...
            while (accept == false && newedit != "-1") {
                if (newedit[0] != 'F') {
                    ...
                    for (int i = 0; i < freightList.size(); i++) {
                        if (freightList[i].getID() == newedit) {
        ...
        case 4:
            ...
            while (accept == false && newedit != "-1") {
                for (int i = 0; i < newedit.size(); i++) {
                    if (isdigit(newedit[i]) == false || newedit.size() != 4) {
